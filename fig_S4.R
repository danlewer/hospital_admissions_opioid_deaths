...
model_order <- c('standard', 'short', 'long', 'endDeath', 'end2019', 'ahm', paste0('farr', 1:10), 'farr_ma')
...
d[, cols := factor(cols, c('standard', 'short', 'long', 'endDeath', 'end2019', 'ahm', 'farr'), col7)]
...
segments(xlims[1], group_seps, xlims[2], group_seps)
...
for(i in 1:2) {
  with(d[m == 'farr_ma'][i], polygon(x = c(l, e, u, e), y = c(o, o-farr_ma_interval, o, o+farr_ma_interval), border = col7[7], col = col7[7]))
}
text(mean(xlims), ylims[1] - 12, 'Conditional odds ratio\nof opioid-related death\n(reference = time in the community)')

# legend

ys <- seq(65, 15, length.out = 7)
arrows(3, ys[-7], 3.4, ys[-7], code = 3, angle = 90, length = 0.07, col = col7[-7])
points(rep(3.2, 6), ys[-7], col = col7[-7], pch = 19)

text(3.7, ys[-7], c('Control days sampled from\n730-28 days before death\n(main analysis)',
                    ...
                    'Analysis limited to deaths with\ndiagnoses of aspiration pneumonia\nor hypoxia'),
     adj = 0)

ys <- seq(5, 10, length.out = 7)
xs <- seq(3.1, 3.3, length.out = 5)
points(xs, ys[-(1:2)], pch = 19, cex = 0.5, col = col7[7])
arrows(xs - 0.2, ys[-(1:2)], xs + 0.2, ys[-(1:2)], length = 0.03, angle = 90, code = 3, col = col7[7])
...
polygon(c(min(xs), mean(xs), max(xs), mean(xs)), c(mean(ys[1:2]), ys[1], mean(ys[1:2]), ys[2]), col = col7[7], border = col7[7])
